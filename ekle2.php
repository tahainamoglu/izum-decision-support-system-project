...
<?php
    ...
    if (isset($_POST["submit"])) {
        $durum = $_POST['durum_ad'];
        $ilce = $_POST['ilce_ad'];
        $yoldur = $_POST['yol_ad'];
        $query = mysqli_query($con, "INSERT INTO mudahale (durum_id,yol_id,ilce_id) VALUES ('$durum','$yoldur',' $ilce')");
        if ($query) {
            echo "<script>alert('eklendi')</script>";
        } else {
            echo "<script>alert('eklenmedi')</script>";
        }
    }
    ?>
<form method="POST">
            ...
            <font color="red">Araç Durumu:</font>
            <select name="durum_ad">
                <?php
                $durum = mysqli_query($con, "SELECT * FROM arac_durumu");

                while ($dur = mysqli_fetch_array($durum)) {
                ?>
                <option value="<?php echo $dur['durum_id'] ?>"><?php echo $dur['durum_ad'] ?></option>
                <?php } ?>
            </select> <br /><br />
             <font color="red">Yol Durumu:</font>
            <select name="yol_ad">
            <?php
                $yoldur = mysqli_query($con, "SELECT * FROM yol_durumu");

                while ($yol = mysqli_fetch_array($yoldur)) {
                ?>
                <option value="<?php echo $yol['yol_id'] ?>"><?php echo $yol['yol_ad'] ?></option>
                <?php } ?>
            </select> <br /><br />
            <font color="red">İlçe Adı:</font>
            <select name="ilce_ad">
                <?php
                $ilce = mysqli_query($con, "SELECT * FROM ilce");

                while ($ilc = mysqli_fetch_array($ilce)) {
                ?>
                <option value="<?php echo $ilc['ilce_id'] ?>"><?php echo $ilc['ilce_ad'] ?></option>
                <?php } ?>
            </select>
